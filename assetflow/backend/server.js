...
app.post('/api/assets/:id/return', authenticateToken, async (req, res) => { try { const asset = await Asset.findOne({ id: req.params.id }); if (!asset) return res.status(404).json({ message: 'Asset not found.' }); const activeAllocation = await Allocation.findOne({ assetId: req.params.id, status: 'Approved' }); const canReturn = normalizeRole(req.user.role) === 'admin' || normalizeRole(req.user.role) === 'assetmanager' || activeAllocation?.allocatedTo === req.user.name || activeAllocation?.allocatedTo === req.user.email; if (activeAllocation && !canReturn) return res.status(403).json({ message: 'You can only return assets allocated to you.' }); await Asset.updateOne({ id: req.params.id }, { $set: { owner: null, status: 'Active' } }); await Allocation.updateMany({ assetId: req.params.id, status: 'Approved' }, { $set: { status: 'Returned', returnedAt: new Date() } }); await createNotification({ title: 'Asset Returned', message: `${asset.name} was returned to department stock.`, type: 'info', targetRole: 'Asset Manager' }); res.json({ success: true, message: 'Asset returned to department stock successfully!' }); } catch (error) { handleError(res, error); } });
...
app.post('/api/allocations/:id/action', authenticateToken, async (req, res) => {
    ...
    try {
        ...
        if (!['Approved', 'Rejected', 'Returned'].includes(status)) return res.status(400).json({ message: 'Invalid allocation status.' });
        if (status === 'Approved') { if (!finalAssetId) return res.status(400).json({ message: 'A specific asset must be assigned to approve this request.' }); const asset = await Asset.findOne({ id: finalAssetId }); if (!asset) return res.status(404).json({ message: 'Asset not found.' }); const existingAllocation = await Allocation.findOne({ assetId: finalAssetId, status: 'Approved', id: { $ne: allocation.id } }); if (existingAllocation || asset.owner) return res.status(409).json({ message: 'This asset is already allocated.' }); allocation.assetId = finalAssetId; allocation.assetName = asset.name; await Asset.updateOne({ id: finalAssetId }, { $set: { owner: allocation.allocatedTo, department: allocation.department, status: 'Active' } }); }
